class Store < ApplicationRecord
  ...
  store :settings, accessors: [
    ...
    :cookie_consent,              # boolean, ask new visitors for cookie consent
  ...
  def stationary_options
    pages.stationary.map { |p| [p.to_s, p.id] }
  end

  # Defines accessors to boolean settings not generated by Rails.
  %w[main_nav_lookup detach_menu masonry fancy_cart cart_promotion favorites cookie_consent category_order admit_guests custom_smtp_settings disable_mail disable_shipment_transfers b2b_sales global_sales smtp_enable_starttls_auto].each do |method|
